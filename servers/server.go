...
func StartManager() {
	go Manager.Start() // 启动 Manager 处理客户端连接
}
...
func AddClient2Group(systemId string, groupName string, clientId string, userId string, extend string) {
	...
	if util.IsCluster() {
		...
		if client, err := Manager.GetByClientId(clientId); err == nil {
			log.Infoln("当前client：", client.ClientId, client.SystemId)
			//如果是单机，就直接添加到本地group了
			Manager.AddClient2LocalGroup(groupName, client, userId, extend)
			log.Infoln("添加到本地group：", groupName)
		} else {
			log.Infoln("client不存在：", err)
		}
	}
}
...
func GetOnlineList(systemId *string, groupName *string) map[string]interface{} {
	...
	if util.IsCluster() {
		...
		//如果是单机服务，则只发送到本机
		t := util.GenGroupKey(*systemId, *groupName)
		retList := Manager.GetGroupClientList(t)
		fmt.Println(Manager.Groups[t])
