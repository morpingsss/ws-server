...
func NewClientManager() (clientManager *ClientManager) {
	clientManager = &ClientManager{
		...
		Groups:        make(map[string][]string), // 确保正确初始化
		SystemClients: make(map[string][]string),
	}
	return
}
...
func (manager *ClientManager) AddClient2LocalGroup(groupName string, client *Client, userId string, extend string) {
	...
	fmt.Println(manager.Groups[groupKey])
...
func (manager *ClientManager) addClient2Group(groupKey string, client *Client) {
	...
	// 初始化 groupKey 对应的切片
	if _, exists := manager.Groups[groupKey]; !exists {
		manager.Groups[groupKey] = []string{}
	}
